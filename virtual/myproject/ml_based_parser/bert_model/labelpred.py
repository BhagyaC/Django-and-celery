...
logger.info('Initialize bert client for LINE LABEL')
bc = ConcurrentBertClient(ip=os.getenv('BERT_SVC_URL', 'bertservice'), port=5555, port_out=5556)
logger.info('Initialize bert client for LINE LABEL completed')
...
def predict_label(json_input: json) -> pd.DataFrame:
    """
    Predicting line label from json input
    :param json_input: JSON representation of line input
    :return: DataFrame containing predicted labels
    """
    logger.info('Predicting LINE LABEL...')
    ...
    model_dir = './ml_based_parser/bert_model/model_label/'
    ...
    logger.debug('Eval: {}'.format(json_input))
    ...
    logger.debug('Predicted Class{}'.format(predicted_classes1))
    df = pd.DataFrame(dict({"features": result, "prediction": predicted_classes1}))
    # df.to_csv("label_predictedtestOutputZeinab.csv", index=False)
    return df
